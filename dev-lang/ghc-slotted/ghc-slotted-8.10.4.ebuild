...
GHC_P="ghc-${GHC_PV}" # using ${P} is almost never correct
...
relocate_ghc() {
	...
	if [[ ${bin_libdir} != $(get_libdir) ]]; then
		...
		relocate_path "/usr/${bin_libdir}" "/usr/$(get_libdir)" \
			...
			"$gp_back" \
			"${WORKDIR}/usr/$(get_libdir)/ghc-${ghc_v}/package.conf.d/"*
	fi
	...
	if use prefix; then
		hprefixify "${bin_libpath}"/ghc*/settings
	fi
...
ghc-tag-file() {
	...
	if [[ -e "${file}" ]]; then
		mv -v "${file}" "${newfile}" || die
	else
		eerror "Expected file does not exist: ${file}"
		eerror "The maintainer of this ebuild should look for changes from upstream."
		die
	fi
}
...
ghc-remove-symlink() {
	...
	if [[ -L "${file}" ]]; then
		...
		eerror "Exists but not a symlink: ${file}"
		eerror "Refusing to remove."
		die
	else
		eerror "Expected symlink does not exist: ${file}"
		eerror "The maintainer of this ebuild should look for changes from upstream."
		die
	fi
}
...
src_prepare() {
	...
	if ! use ghcbootstrap && [[ ${CHOST} != *-darwin* && ${CHOST} != *-solaris* ]]; then
		...
		pax-mark -m "${WORKDIR}/usr/$(get_libdir)/ghc-${BIN_PV}/bin/ghc"
	fi
	...
	if use binary; then
		...
		eapply "${FILESDIR}"/ghc-7.0.4-CHOST-prefix.patch
		eapply "${FILESDIR}"/ghc-8.2.1-darwin.patch
		eapply "${FILESDIR}"/ghc-7.8.3-prim-lm.patch
		eapply "${FILESDIR}"/ghc-8.8.1-revert-CPP.patch
		eapply "${FILESDIR}"/ghc-8.10.1-allow-cross-bootstrap.patch
		eapply "${FILESDIR}"/ghc-8.10.3-C99-typo-ac270.patch

		# a bunch of crosscompiler patches
		# needs newer version:
		#eapply "${FILESDIR}"/ghc-8.2.1_rc1-hp2ps-cross.patch

		# mingw32 target
		pushd "${S}/libraries/Win32"
			eapply "${FILESDIR}"/ghc-8.2.1_rc1-win32-cross-2-hack.patch # bad workaround
		popd
...
src_install() {
	if use binary; then
		...
		if ! is_crosscompile; then
			newbashcomp "${FILESDIR}/ghc-bash-completion" "ghc-pkg-${PV}"
			newbashcomp "utils/completion/ghc.bash" "ghc-${PV}"
		fi
	fi
	...
	# Remove symlinks for various executables
	einfo "Removing symlinks in binary directory"
	local file=
	for file in ghc ghc-pkg ghci runghc runhaskell; do
		ghc-remove-symlink "${ED}/usr/bin/${file}"
	done

	# Tag misc bin files with GHC version
	local file=
	for file in hp2ps hpc hsc2hs; do
		ghc-tag-file \
			"${ED}/usr/bin/${file}" \
			"${ED}/usr/bin/${file}-${PV}"
	done

	# Tag man file with GHC version
	ghc-tag-file \
		"${ED}/usr/share/man/man1/ghc.1" \
		"${ED}/usr/share/man/man1/ghc-${PV}.1"
}
